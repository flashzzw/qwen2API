...
CORS_ALLOWED_ORIGINS=http://localhost:5173,http://127.0.0.1:5173  # 独立前端部署时改成你的前端域名
...
| `CORS_ALLOWED_ORIGINS` | `http://localhost:5173,http://127.0.0.1:5173,...` | 允许携带管理员 Cookie 的前端来源 | 前后端分离或 Vercel 部署时必须包含实际前端域名 |
...
| `CORS_ALLOWED_ORIGINS` | 本地 Vite/Preview 地址 | 允许管理台跨域携带 Cookie 的来源列表，逗号分隔。 |
| `PORT` | `7860` | 后端服务监听端口。 |
...
但正式部署建议始终创建 `.env`，至少设置 `ADMIN_KEY`；如果前端独立部署，还要把 `CORS_ALLOWED_ORIGINS` 设为实际前端域名。
...
- 如果前后端分离部署，确认后端 `CORS_ALLOWED_ORIGINS` 已包含当前前端域名。
