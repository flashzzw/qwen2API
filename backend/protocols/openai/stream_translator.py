...
from backend.protocols.common.standard_request import CLAUDE_CODE_OPENAI_PROFILE, OPENCLAW_OPENAI_PROFILE
...
class OpenAIStreamTranslator:
    ...
    @staticmethod
    def _tool_calls_from_directive(directive: RuntimeToolDirective) -> list[dict[str, Any]]:
        return [
            {
                "id": block["id"],
                "name": block["name"],
    ...
    def finalize(self, finish_reason: str, final_directive: RuntimeToolDirective | None = None) -> list[str]:
        final_finish_reason = finish_reason
        buffered_text = "".join(self.buffered_toolish_fragments)
        if not self.tool_calls_emitted:
            directive = final_directive
            should_emit_tool_calls = False
            if directive is not None:
                should_emit_tool_calls = directive.stop_reason == "tool_use"
            elif self.build_final_directive is not None:
                directive = self.build_final_directive("".join(self.answer_fragments))
                should_emit_tool_calls = self._should_finalize_tool_calls(directive)

            if directive is not None and should_emit_tool_calls:
                self._discard_pending_content_chunks()
                tool_calls = self._tool_calls_from_directive(directive)
                if tool_calls:
                    self.emit_tool_calls(tool_calls)
